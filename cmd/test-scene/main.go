...
func genWorld() *rtc.WorldT {
	...
	floor := rtc.Sphere()
	floor.SetTransform(rtc.Scaling(10, 0.01, 10))
	floor.Material().Color = rtc.Color(1, 0.9, 0.9)
	floor.Material().Specular = 0

	leftWall := rtc.Sphere()
	leftWall.SetTransform(rtc.M4Identity().Scale(10, 0.01, 10).RotateX(math.Pi/2).RotateY(-math.Pi/4).Translate(0, 0, 5))
	leftWall.SetMaterial(*floor.Material())

	rightWall := rtc.Sphere()
	rightWall.SetTransform(rtc.M4Identity().Scale(10, 0.01, 10).RotateX(math.Pi/2).RotateY(math.Pi/4).Translate(0, 0, 5))
